...
function formatRelativeTime(date: Date): string {
  ...
  if (seconds < 5) return m.time_just_now();

  if (isInFuture) {
    let time = "";
    if (seconds < 60)
      time = seconds === 1 ? m.unit_second() : m.unit_seconds({ count: seconds });
    else if (minutes < 60)
      time =
        minutes === 1 ? m.unit_minute() : m.unit_minutes({ count: minutes });
    else if (hours < 24)
      time = hours === 1 ? m.unit_hour() : m.unit_hours({ count: hours });
    else if (days < 7)
      time = days === 1 ? m.unit_day() : m.unit_days({ count: days });
    else return date.toLocaleDateString();

    return m.time_in({ time });
  }
  ...
  if (minutes < 60) {
    return m.time_ago({
      time: m.unit_minute_seconds({ minutes, seconds: seconds % 60 }),
    });
  }
