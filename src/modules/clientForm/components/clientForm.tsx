...
import { formatClientFormData, submitClientForm } from "../logic/functions";
import {
  beneficiaryRelation,
  clientFormSchema,
  employerStatus,
...
/** Auto-format raw digits into SSN format (XXX-XX-XXXX). */
...
/** Auto-format raw digits into phone format ((XXX) XXX-XXXX). */
...
export function ClientForm() {
  ...
  const onSubmit = async (data: ClientFormSchema) => {
    ...
    setLoading(true);
    try {
      const formatted = await submitClientForm(data);
                ...
                {beneficiariesFieldArray.fields.map((field, index) => (
                  ...
                  >
                    ...
                    <div className="grid gap-3 sm:gap-4 sm:grid-cols-2">
                      ...
                      <InputElement
                        control={form.control}
                        name={`beneficiaries.${index}.birthDay`}
                        label={m.client_form_beneficiary_birth_day()}
                        placeholder={m.client_form_beneficiary_birth_day_placeholder()}
